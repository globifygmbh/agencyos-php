...
<div x-data="profileApp()" x-init="init()">
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="glass-card rounded-2xl p-6">
            <div class="flex items-start gap-6">
                <div class="relative">
                    ...
                    <template x-if="!user || !user.profile_image">
                        <div class="w-20 h-20 rounded-2xl flex items-center justify-center text-white text-2xl font-bold"
                             style="background: #009dde;">
                            <?= strtoupper(substr($currentUser['first_name'] ?? '', 0, 1)) . strtoupper(substr($currentUser['last_name'] ?? '', 0, 1)) ?>
                        </div>
                    </template>
                    <label class="absolute -bottom-2 -right-2 w-7 h-7 bg-white/8 border border-white/10 rounded-full flex items-center justify-center cursor-pointer hover:bg-white/12 transition-colors">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-sm text-white/40 mt-0.5" x-text="user?.email || ''"></div>
                    <span class="inline-block mt-2 text-xs px-2.5 py-1 rounded-full bg-[#009dde]/10"
                          style="color: #009dde;"
        ...
        <!-- Personal Info -->
        <div class="glass-card rounded-2xl p-6">
            <h3 class="text-sm font-heading font-semibold text-white mb-4">Persönliche Informationen</h3>
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-white/40 mb-1.5">Vorname</label>
                        <input x-model="form.first_name" type="text" class="input-field">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-white/40 mb-1.5">Nachname</label>
                        <input x-model="form.last_name" type="text" class="input-field">
                    </div>
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input x-model="form.birthday" type="date" class="input-field">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-white/40 mb-1.5">Position / Jobtitel</label>
                    <input x-model="form.job_title" type="text" class="input-field">
                </div>
                <button @click="saveProfile()" x-show="saved !== null" class="btn-primary">
        ...
        <!-- Change Password -->
        <div class="glass-card rounded-2xl p-6">
            <h3 class="text-sm font-heading font-semibold text-white mb-4">Passwort ändern</h3>
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-medium text-white/40 mb-1.5">Aktuelles Passwort</label>
                    <input x-model="pwForm.old_password" type="password" class="input-field">
                </div>
                <div>
                    <label class="block text-xs font-medium text-white/40 mb-1.5">Neues Passwort</label>
                    <input x-model="pwForm.new_password" type="password" class="input-field">
                </div>
        ...
        <!-- Sessions -->
        <div class="glass-card rounded-2xl p-6">
            <h3 class="text-sm font-heading font-semibold text-white mb-4">Aktive Sessions</h3>
            <div class="space-y-2">
                <template x-for="s in sessions" :key="s.token">
                    <div class="flex items-center gap-3 py-2 border-b border-white/10 last:border-0">
                        <div class="flex-1 text-xs text-white/40">
                            <div class="font-mono" x-text="s.token"></div>
                            <div class="mt-0.5" x-text="(s.ip_address || '') + ' · ' + formatDate(s.created_at)"></div>
                        </div>
                        <div class="text-xs text-white/50" x-text="'Läuft ab: ' + formatDate(s.expires_at)"></div>
                    </div>
                </template>
            </div>
            <button @click="logoutAll()"
                    class="mt-4 px-4 py-2 rounded-xl text-red-400 hover:bg-red-500/10 transition-colors text-sm font-medium">
